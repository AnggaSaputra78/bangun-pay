...
const ExpensesListPage = () => {
  ...
  const stats = useMemo(() => {
    if (expenses.length === 0) return { totalAmount: 0, count: 0, avgAmount: 0, maxAmount: 0 }
  ...
  const handleReset = () => {
    setFilters({ search: '', projectId: '', categoryId: '', paymentMethod: '', startDate: '', endDate: '' })
    setPage(1)
  }
  ...
  const handleDelete = (id, name) => {
    if (window.confirm(`Hapus transaksi "${name}"?`)) deleteMutation.mutate(id)
  }
  const handleExport = () => {
    if (expenses.length === 0) return toast.error('Tidak ada data')
    const headers = ['Tanggal', 'Nama', 'Proyek', 'Kategori', 'Metode', 'Nominal']
    const rows = expenses.map((e) => [
      new Date(e.expenseDate).toLocaleDateString('id-ID'), e.name, e.projectId?.name || '',
      e.categoryId?.name || '', e.paymentMethod, e.amount,
    ])
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <ExpenseFilterBar filters={filters} onFilterChange={handleFilterChange} onReset={handleReset} onExport={handleExport} projects={projectsData || []} categories={categoriesData || []} />

      <div className="card overflow-hidden p-0">
        <ExpenseTable expenses={expenses} onSort={handleSort} sortBy={sortBy} sortOrder={sortOrder} onView={setSelectedExpense} onDelete={handleDelete} isLoading={isLoading} />
        {pagination.pages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-navy-700">
            <p className="text-sm text-gray-500">{(pagination.page - 1) * 20 + 1}-{Math.min(pagination.page * 20, pagination.total)} dari {pagination.total}</p>
            <div className="flex gap-2">
              <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page === 1} className="btn btn-secondary py-1.5 px-3 text-xs">Sebelumnya</button>
              <span className="text-sm text-gray-600 px-2">{page} / {pagination.pages}</span>
              <button onClick={() => setPage(Math.min(pagination.pages, page + 1))} disabled={page === pagination.pages} className="btn btn-secondary py-1.5 px-3 text-xs">Selanjutnya</button>
            </div>
          </div>
        )}
      </div>
